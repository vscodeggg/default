    ...
    def transcribe(self, audio_path: str) -> str:
        with open(audio_path, "rb") as audio_file:
            result = self.client.audio.transcriptions.create(
                ...
                temperature=0,
                prompt=(
                    "Clinical dental charting dictation in English. "
                    "Vocabulary: tooth number, occlusal, incisal, mesial, distal, buccal, "
                    "lingual, facial, caries, cavity, composite, resin, amalgam, crown, "
                    "root canal, implant, fracture, probing depth, pocket, millimeters, "
                    "bleeding on probing."
                ),
            )
        return sanitize_transcript(result.text)
